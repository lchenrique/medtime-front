...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    const Comp = "button";
    const SubComp = asChild ? Slot : "div";
    return (
      <Comp
      ...
      >
        {loading ? (
          <ClipLoader
            loading
            className="border-[hsl(var(--primary))_hsl(var(--primary))]"
            color="inherit"
            size={20}
          />
        ):
        <SubComp className="inline-flex gap-2 items-center justify-center whitespace-nowrap rounded-md ">
          {children}
        </SubComp> }
      </Comp>
    );
